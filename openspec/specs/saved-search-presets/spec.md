...
### Requirement: Search opens with default filters

The advanced search page SHALL open with default filters and SHALL run its initial search using
those defaults. It SHALL NOT read or write a browser copy of the last-used filters. A saved search
SHALL affect the form only when the actor explicitly selects it.

#### Scenario: User returns in the same browser

- **WHEN** a user opens search after previously running a filtered search
- **THEN** the form and initial search use default filters, even if old last-filter data remains
  in browser storage

#### Scenario: User selects a saved search

- **WHEN** a user selects a preset from the shared library
- **THEN** its filters populate the form and the page searches with them
