...
physically removed. The API SHALL permit it to be deactivated, so that it stops being offered
for new selections while existing candidate records keep their meaning.

Until candidate relations are owned by the API, the administration screen SHALL refuse to
deactivate a value it can see is in use, so that an administrator cannot retire a value that
browser-held candidate records still depend on. This screen-level refusal is transitional: it
is removed once the API can evaluate the rule itself, after which the API's permission to
deactivate becomes the product's behavior.
...
#### Scenario: Administration screen refuses to deactivate a value in use

- **WHEN** an administrator deactivates a value on the administration screen while candidate
  records that the screen can see still reference it
- **THEN** the screen refuses the change with the Spanish message
  `No se puede desactivar: el valor esta en uso por candidatos.` and the value stays active
