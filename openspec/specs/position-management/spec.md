...
The closed sortable fields SHALL be `title`, `location`, `status` and `updatedAt`, each in `asc` or `desc` direction. The position identifier ascending SHALL be the final tie-breaker. Each list item SHALL contain only identifier, title, location, status, update time and version; it SHALL NOT contain description, requirements or candidate-match data.
...
- **THEN** the candidate appears in the live search without a position-candidate association being created
...
- **THEN** the existing permission-checked document action is available without exposing a storage path
