...
added item SHALL start with an empty level, meaning any level. With a required level, adding a value
SHALL open that item's level editor immediately. The item SHALL be committed only once a level is
chosen. Closing the editor without a level SHALL discard the item and commit nothing.
...
- **WHEN** a value is added to a picker whose level is required and the user chooses a level
- **THEN** the item is committed with that level

#### Scenario: Required level is abandoned

- **WHEN** a value is added to a picker whose level is required and the user closes the editor
  without choosing a level
- **THEN** no item remains and nothing is committed
...
Wherever a picker is disabled because catalogs cannot be loaded, the page SHALL show the existing
