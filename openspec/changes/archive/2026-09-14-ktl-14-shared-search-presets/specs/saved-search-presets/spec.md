...
**Reason**: Presets become a single shared library curated by administrators; per-owner privacy no
longer applies.

**Migration**: Replaced by "Shared preset listing and retrieval". Existing owner-scoped presets are
discarded (see "Existing private presets are discarded") and must be recreated by an actor holding
`manage_presets` in the presets administration section.
