...
5. **Evaluar si la historia ya está completa** según las buenas prácticas de producto. Una historia completa para este proyecto incluye:
   ...
   - Cobertura de pruebas: unitarias/integración con Vitest (y React Testing Library para componentes) en [tests/unit/](tests/unit/) y [tests/integration/](tests/integration/), e2e con Playwright en [tests/e2e/](tests/e2e/), pruebas xUnit unitarias e integración con PostgreSQL real en [backend/Tests/](backend/Tests/), y comprobaciones de seguridad en [tests/security/](tests/security/) cuando se toquen permisos, grants o almacenamiento.
