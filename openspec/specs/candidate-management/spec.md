...
The system SHALL record document metadata — type, original filename, media type, size, primary
flag and upload timestamp — against a candidate, and SHALL keep at most one document marked
primary per candidate. Document metadata SHALL NOT expose an internal storage path or key.

#### Scenario: Document metadata is attached

- **WHEN** an authorized actor attaches a document to a candidate
